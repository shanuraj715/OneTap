...
const track: CSSProperties = { background: "var(--color-border)", overflow: "hidden" };
const label: CSSProperties = { fontSize: "0.78rem", color: "var(--color-text-muted)", display: "flex", justifyContent: "space-between", marginBottom: 6 };

/** 01 plain bar */
export function ProgressBar({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  return (
    <div>
...
export function ProgressThick({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  return (
...
export function ProgressLine({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  return (
    <div>
      {text ? <div style={label}><span>{text}</span><span>{v}%</span></div> : null}
...
/** 04 striped */
export function ProgressStriped({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  return (
    <div>
      {text ? <div style={label}><span>{text}</span><span>{v}%</span></div> : null}
...
export function ProgressSegments({ value, label: text }: ProgressProps) {
  const v = clamp(value);
...
/** 06 circular dial */
export function ProgressRing({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  const r = 26;
  const c = 2 * Math.PI * r;
  return (
...
export function ProgressSteps({ value, label: text }: ProgressProps) {
  const v = clamp(value);
...
export function ProgressGradient({ value, label: text }: ProgressProps) {
  const v = clamp(value);
  return (
    <div>
      {text ? <div style={label}><span>{text}</span><span>{v}%</span></div> : null}
