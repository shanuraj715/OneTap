...
import { toneVars, type AlertProps } from "./types";

const ICONS = { info: "i", success: "✓", warning: "!", danger: "×" } as const;

const base: CSSProperties = { padding: "14px 16px", fontSize: "0.88rem", lineHeight: 1.5 };
const titleStyle: CSSProperties = { fontWeight: 700, marginBottom: 3 };

/** 01 left accent bar over a wash */
export function AlertLeftAccent({ title, children, tone = "info" }: AlertProps) {
...
export function AlertSoft({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
...
/** 03 outline only */
export function AlertOutline({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
...
export function AlertIcon({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
...
/** 05 solid banner */
export function AlertBanner({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
...
/** 06 top accent rule */
export function AlertTopRule({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
...
export function AlertInline({ title, children, tone = "info" }: AlertProps) {
  const t = toneVars(tone);
  return (
