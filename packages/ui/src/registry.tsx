...
import type { MenuItem } from "@onetap/config-schema";
import type { ItemCardProps } from "./cards";
import * as cards from "./cards";
import type { FooterProps } from "./footers";
import * as footers from "./footers";
import type { HeaderProps } from "./headers";
...
export interface VariantMeta<P = any> {
  id: string;
  code: string;
  name: string;
  description: string;
...
export interface CardVariantMeta extends VariantMeta<ItemCardProps> {
  layout: "grid" | "list";
  minWidth: number;
}
...
export interface VariantSlot {
  /** the field in config.layout this slot writes to */
  key: string;
  label: string;
  description: string;
  /** render edge-to-edge in the gallery (headers, footers) */
  flush?: boolean;
  variants: VariantMeta[];
  preview: (meta: VariantMeta, ctx: PreviewContext) => ReactNode;
}

/* ------------------------------------------------------------------- helpers */

const v = <P,>(id: string, code: string, name: string, description: string, Component: ComponentType<P>): VariantMeta<P> => ({
...
export const headerVariants: VariantMeta<HeaderProps>[] = [
...
export const footerVariants: VariantMeta<FooterProps>[] = [
...
export const itemCardVariants: CardVariantMeta[] = [
...
const headerProps = (ctx: PreviewContext): HeaderProps => ({
  name: ctx.outletName,
  tagline: ctx.tagline,
  phone: ctx.phone,
  links: [
...
const footerProps = (ctx: PreviewContext): FooterProps => ({
  name: ctx.outletName,
  address: ctx.address,
  phone: ctx.phone,
...
export const VARIANT_SLOTS: VariantSlot[] = [
  ...
  },
  {
    ...
    preview: (meta, ctx) => {
      const m = meta as CardVariantMeta;
      const C = m.Component;
      const style =
        m.layout === "list"
          ? { display: "flex", flexDirection: "column" as const, gap: m.id === "card.menu-line" ? 0 : 12 }
...
export const slotByKey = (key: string) => VARIANT_SLOTS.find((s) => s.key === key);

/** Every variant across every family. */
export const allVariants: VariantMeta[] = VARIANT_SLOTS.flatMap((s) => s.variants);
 ...
 */
function pick<T extends { id: string; code: string }>(list: T[], ref: string | undefined): T {
...
export const getHeaderVariant = (ref?: string) => pick(headerVariants, ref);
export const getFooterVariant = (ref?: string) => pick(footerVariants, ref);
export const getItemCardVariant = (ref?: string) => pick(itemCardVariants, ref);
export const getVariant = (slotKey: string, ref?: string) => pick(slotByKey(slotKey)?.variants ?? allVariants, ref);
