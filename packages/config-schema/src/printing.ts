...
export const PAPER_WIDTHS = ["58mm", "80mm", "a5", "a4"] as const;
export const paperWidthSchema = z.enum(PAPER_WIDTHS);
export type PaperWidth = (typeof PAPER_WIDTHS)[number];

export interface PaperMeta {
  label: string;
  /** printable width in mm — thermal rolls print narrower than the roll itself */
  printableMm: number;
  /** printhead dots across the printable area; a raster logo must match this */
  dots: number;
  /** monospace columns at the printer's default font — the ESC/POS layout grid */
  chars: number;
  /** thermal rolls are continuous; sheets have a fixed height */
  continuous: boolean;
  heightMm?: number;
  hint: string;
}

export const PAPER: Record<PaperWidth, PaperMeta> = {
  "58mm": {
    label: '58 mm roll (2")',
    printableMm: 48,
    dots: 384,
...
export const isThermal = (w: PaperWidth): boolean => PAPER[w].continuous;

/* ---------------------------------------------------------------- stations */

/** Where a printer physically sits. Drives which documents get routed to it. */
export const PRINT_STATIONS = ["counter", "kitchen", "bar", "packing", "manager"] as const;
...
export const STATION_HINTS: Record<PrintStation, string> = {
...
export const PRINT_DOCS = ["kot", "receipt", "bill", "refund"] as const;
export const printDocSchema = z.enum(PRINT_DOCS);
export type PrintDocType = (typeof PRINT_DOCS)[number];

export const DOC_LABELS: Record<PrintDocType, string> = {
...
export const DOC_HINTS: Record<PrintDocType, string> = {
...
export const STATION_DEFAULT_DOC: Record<PrintStation, PrintDocType> = {
  counter: "receipt",
  kitchen: "kot",
  bar: "kot",
  packing: "receipt",
 ...
 */
export const PRINT_TARGETS = ["browser", "epos-lan", "cloud", "agent"] as const;
export const printTargetSchema = z.enum(PRINT_TARGETS);
export type PrintTarget = (typeof PRINT_TARGETS)[number];

export interface TargetMeta {
  label: string;
  /** true if the job can print with nobody clicking anything */
  silent: boolean;
  /** true if the printer can be told to cut and kick the cash drawer */
  escpos: boolean;
  hint: string;
  caveat: string;
}

export const TARGETS: Record<PrintTarget, TargetMeta> = {
  browser: {
...
export type PrinterConnection = z.infer<typeof connectionSchema>;
...
export const isJobOpen = (s: PrintJobStatus): boolean => s === "queued" || s === "printing";
 ...
 */
export const RETRY_BACKOFF_SEC = [10, 30, 120, 300, 900] as const;
export const DEFAULT_MAX_ATTEMPTS = 5;

export function nextRetryDelaySec(attempt: number): number {
...
export type PrintBlockKey = (typeof PRINT_BLOCKS)[number];

export const BLOCK_LABELS: Record<PrintBlockKey, string> = {
  logo: "Logo",
...
export const BLOCK_HINTS: Record<PrintBlockKey, string> = {
...
export type PrintAlign = (typeof ALIGNS)[number];
...
export function charsFor(t: Pick<PrintTemplate, "paperWidth" | "charsPerLine">): number {
...
const b = (
  key: PrintBlockKey,
  extra: Partial<Omit<BlockConfig, "key">> = {},
): BlockConfig => blockConfigSchema.parse({ key, ...extra });

/** The starting point for a new outlet — sensible, printable, and editable. */
export function defaultBlocks(docType: PrintDocType): BlockConfig[] {
  if (docType === "kot") {
    return [
...
export function defaultTemplate(docType: PrintDocType, paperWidth: PaperWidth = "80mm"): PrintTemplate {
