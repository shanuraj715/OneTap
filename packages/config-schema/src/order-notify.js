 ...
 */
export const NOTIFY_CHANNELS = ["whatsapp", "sms"]         ;
...
export const NOTIFY_VARIABLE_HINTS                                 = {
...
const templateMap =                         (shape   ) => z.record(z.string(), shape).default({});
...
export function renderTemplate(body        , values                                )         {
  return body.replace(/\{\{(\w+)\}\}/g, (m, key        ) => values[key                  ] ?? m);
}

/** A sample set of values for previewing a template in the admin. */
export function sampleNotifyValues(outletName        )                                 {
  return {
    customerName: "Aarav",
    orderNumber: "0042",
    statusLabel: "Ready",
