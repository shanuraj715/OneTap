...
export function coinsEarnedFor(paidPaise: number, settings: WalletSettings): number {
...
export function coinsValuePaise(coins: number, settings: WalletSettings): number {
  return Math.max(0, coins) * settings.redeemValuePaise;
}

export interface RedeemCheck {
  ok: boolean;
  /** coins actually applied — may be less than requested if the cap trims it */
  coinsApplied: number;
  /** paise value of coinsApplied */
...
export function checkRedeem(
  ...
  const deny = (reason: string): RedeemCheck => ({ ok: false, coinsApplied: 0, discountPaise: 0, reason });
