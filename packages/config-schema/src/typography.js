...
const LETTER_SPACING = { tight: "-0.02em", normal: "0", wide: "0.04em" }         ;
...
export function typographyCss(t            )         {
  const scale = TYPE_SCALES[t.scale];
  const step = (n        ) => `${Math.round(scale.ratio ** n * 1000) / 1000}rem`;
  return [
    ":root {",
