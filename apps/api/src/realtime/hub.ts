...
import { OutletModel, type UserDoc } from "@onetap/db";
...
async function authorize(
  req: IncomingMessage,
): Promise<{ user: UserDoc; brandId: string; outletId: string } | null> {
 ...
 */
export function broadcast(outletId: string, event: ServerEvent): void {
  for (const client of clients) {
    if (client.outletId === outletId) send(client, event);
  }
}

export function connectionCount(outletId?: string): number {
