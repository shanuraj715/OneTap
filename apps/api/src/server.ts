...
import { dispatchDue } from "./modules/printing/printing.service";
 ...
 */
function startPrintDispatcher(): void {
  let running = false;
  printTimer = setInterval(() => {
    if (running) return; // never overlap a slow batch with the next tick
    running = true;
    dispatchDue()
      .catch((err: Error) => logger.error({ err }, "print dispatcher tick failed"))
      .finally(() => {
        running = false;
      });
