...
import { agentToken, verifyAgentToken } from "./agent-auth";
import {
  buildRenderInput,
  cancelJob,
  claimJobs,
...
printingRouter.post("/jobs/claim", async (req, res) => {
  ...
  const allowed: PrintTarget[] = body.targets.filter((t) => t === "browser" || t === "epos-lan");
...
async function agentContext(req: import("express").Request) {
