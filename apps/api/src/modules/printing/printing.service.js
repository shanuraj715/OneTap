...
export function shapePrinter(p            )                                                                    {
  const { cloudApiKey, ...connection } = p.connection ?? ({}                            );
  return {
    id: String(p._id),
    name: p.name,
    station: p.station,
...
export async function createPrinter(ctx               , input              ) {
...
export async function updatePrinter(ctx               , id        , patch                       ) {
...
export async function deletePrinter(ctx               , id        ) {
  const printer = await getPrinterDoc(ctx, id);
...
export async function ensureDefaultTemplates(ctx               ) {
 ...
 */
export async function templateForPrinter(ctx               , printer            )                         {
...
async function outletIdentity(ctx               )                        {
...
async function toRenderOrder(ctx               , order          )                       {
  let tableNumber                    ;
  if (order.tableId) {
...
async function qrForTemplate(template               )                              {
 ...
 */
export async function autoPrintForOrder(ctx               , order          , status             )                  {
...
export async function printStatusForOrders(ctx               , orderIds          ) {
...
export async function getJobDoc(ctx               , id        ) {
...
export async function retryJob(ctx               , id        , by         )                        {
  const job = await getJobDoc(ctx, id);
 ...
 */
export async function reprintJob(ctx               , id        , by         )                        {
...
export async function testPrint(ctx               , printerId        , by         )                        {
  const printer = await getPrinterDoc(ctx, printerId);
  const job = await enqueueJob({
    ctx,
    printer,
...
export async function dispatchDue(limit = 10)                                                 {
  ...
  for (const job of due) {
    const ctx                = { brandId: job.brandId, outletId: job.outletId };
    const printer = await PrinterModel.findOne(tenantFilter(ctx, { _id: job.printerId }));
    if (!printer) {
