import { charsFor, PAPER, type BlockConfig, type PrintSize } from "@onetap/config-schema";
import { renderBlocks, type RenderInput } from "./render";
...
const ALIGN = (a: BlockConfig["align"]) =>
  Buffer.from([ESC, 0x61, a === "center" ? 1 : a === "right" ? 2 : 0]);
const BOLD = (on: boolean) => Buffer.from([ESC, 0x45, on ? 1 : 0]);
const FEED = (lines: number) => Buffer.from([ESC, 0x64, Math.max(0, Math.min(255, lines))]);
 ...
 */
const MAGNIFY: Record<PrintSize, number> = {
  sm: 0x00, // 1x1
  md: 0x00, // 1x1
  lg: 0x11, // 2x2
  xl: 0x22, // 3x3
};
const SIZE = (s: PrintSize) => Buffer.from([GS, 0x21, MAGNIFY[s]]);

/** How many columns a magnified block actually gets. */
const widthFactor = (s: PrintSize): number => (MAGNIFY[s] >> 4) + 1;
 ...
 */
function raster(width: number, height: number, data: Buffer): Buffer {
 ...
 */
function qrCode(text: string, moduleSize = 6): Buffer {
...
function toCp437(s: string): Buffer {
  const cleaned = s
    .replace(/₹/g, "Rs.")
    .replace(/[‘’]/g, "'")
    .replace(/[“”]/g, '"')
...
function refit(line: string, cols: number): string[] {
 ...
 */
export function toEscPos(input: RenderInput, opts: EscPosOptions = {}): Buffer {
  const { template } = input;
  const baseCols = charsFor(template);
  const parts: Buffer[] = [INIT, CODEPAGE];
 ...
 */
export function toEposXml(bytes: Buffer): string {
