...
export function shapeCoupon(d: CouponDoc): Coupon {
...
export async function listCoupons(ctx: TenantContext): Promise<Coupon[]> {
  const docs = await CouponModel.find(tenantFilter(ctx)).sort({ active: -1, createdAt: -1 });
  return docs.map((d) => shapeCoupon(d));
}

export async function createCoupon(ctx: TenantContext, input: CouponInput): Promise<Coupon> {
...
export async function updateCoupon(ctx: TenantContext, id: string, patch: Partial<CouponInput>): Promise<Coupon> {
...
export async function deleteCoupon(ctx: TenantContext, id: string): Promise<void> {
...
function validateCouponShape(c: CouponInput): void {
 ...
 * order.
 */
export async function checkCoupon(input: CouponCheckInput): Promise<CouponCheck> {
  ...
  const context: CouponContext = { subtotal, channel };
...
export async function publicCoupons(ctx: TenantContext): Promise<Coupon[]> {
  const now = Date.now();
