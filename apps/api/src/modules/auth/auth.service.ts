...
import { SessionModel, UserModel, type UserDoc } from "@onetap/db";
...
export async function userFromToken(token: string | undefined): Promise<UserDoc | null> {
...
export function toSessionUser(user: UserDoc): SessionUser {
  ...
  const role: Role | null = user.isSuperAdmin ? "super_admin" : (membership?.role ?? null);
  return {
    id: String(user._id),
    email: user.email,
    name: user.name,
