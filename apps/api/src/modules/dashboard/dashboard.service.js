import { evaluateSla,                     } from "@onetap/config-schema";
import {
...
import { outletConfigFor, QUEUE_STATUSES, UNPAID_PREPAID } from "../orders/orders.service.js";
 ...
 */
export async function getDashboardStats(ctx               )                          {
  ...
  const missedOrders = activeOrders.filter((o) => evaluateSla(o         , config.operations.sla) !== null).length;

  const revenueByDay = new Map(revenueTrendRows.map((r) => [r._id          , r                                       ]));
  ...
  const hourByBucket = new Map(hourRows.map((r) => [r._id          , r.n          ]));
  ...
  return {
    ...
    revenueTrend,
    ordersByChannel: channelRows.map((r) => ({ channel: r._id          , count: r.n           })),
    ordersByStatus: statusRows.map((r) => ({ status: r._id          , count: r.n           })),
    ordersByHour,
    topItems: topItemRows.map((r) => ({ name: r._id          , quantity: r.quantity          , revenue: r.revenue           })),
