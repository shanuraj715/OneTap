...
import { z } from "zod";
import type { Permission } from "@onetap/config-schema";
import { isProd } from "../../env";
import { requireBrandContext, requireOutletContext } from "../../middleware/auth";
...
const AREA_PERMISSIONS: Record<string, Permission> = {
  theme: "theme:update",
...
outletsRouter.patch("/:id/config", async (req, res) => {
  ...
  const needed: Permission[] = areas.map((a) => AREA_PERMISSIONS[a] ?? "outlet:update");
