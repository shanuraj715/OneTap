...
import { ensureOwner } from "../auth/auth.service";
import { clearMenu, seedGazabMomosMenu } from "../menu/menu.seed";
 ...
 */
function byBrand(ctx: TenantContext, extra: Record<string, unknown> = {}) {
 ...
 */
function withDefaults<T extends { config?: unknown }>(outlet: T): T {
  return { ...outlet, config: outletConfigSchema.parse(outlet.config ?? {}) };
}

/** Outlets for one brand — tenant-scoped. */
export async function listOutlets(ctx: TenantContext) {
...
export async function getOutletById(ctx: TenantContext, id: string) {
...
export async function resolveOutlet(opts: {
  ...
  if (opts.host) {
    ...
    if (byHost) return withDefaults(byHost) as OutletDoc;
  }
  ...
  if (opts.slug?.includes("/")) {
    ...
    return bySlug ? (withDefaults(bySlug) as OutletDoc) : null;
  }
 ...
 */
export async function seedGazabMomos(opts: { reset?: boolean } = {}) {
