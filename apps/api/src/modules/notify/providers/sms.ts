...
function fill(template: string, vars: Record<string, string>, encode: boolean): string {
 ...
 */
export const smsProvider: NotifyProvider = {
  id: "sms",
  ...
  async send(creds: NotifyCredentials, input: SendInput): Promise<SendResult> {
    ...
    try {
      ...
      return { ok: false, reason: (e as Error).message || "Could not reach the SMS gateway" };
    }
