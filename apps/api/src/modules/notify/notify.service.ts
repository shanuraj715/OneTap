...
import { providerFor, type NotifyCredentials } from "./providers";

/* -------------------------------------------------------------- credentials */

async function credentialsFor(ctx: TenantContext, channel: NotifyOrderChannel): Promise<NotifyCredentials> {
  const doc = await NotificationCredentialModel.findOne(tenantFilter(ctx, { channel })).lean();
  if (!doc) return {};

  const creds: NotifyCredentials = { ...(doc.publicFields ?? {}) };
...
export async function listNotifyConfig(ctx: TenantContext) {
...
export async function clearNotifyCredentials(ctx: TenantContext, channel: NotifyOrderChannel) {
 ...
 */
export async function fireOrderNotifications(ctx: TenantContext, order: OrderDoc, status: OrderStatus): Promise<void> {
  ...
  for (const channel of NOTIFY_CHANNELS) {
    ...
    try {
      ...
      const values: Record<NotifyVariable, string> = {
      ...
      let result;
      if (channel === "whatsapp") {
        const wa = template as { templateName?: string; languageCode?: string } | undefined;
        if (!wa?.templateName) {
        ...
        const sms = template as { body?: string } | undefined;
        if (!sms?.body) {
      ...
      await log(ctx, { channel, event: status, orderId, orderNumber: order.orderNumber, to, status: "failed", error: (e as Error).message || "Unexpected error" });
    }
  }
}
...
export async function listNotificationLogs(
  ctx: TenantContext,
  opts: { channel?: NotifyOrderChannel; event?: OrderStatus; status?: NotificationLogStatus; limit?: number } = {},
) {
