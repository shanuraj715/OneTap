...
import { checkCoupon, recordRedemption } from "../coupons/coupons.service";
import { getMenu } from "../menu/menu.service";
import { fireOrderNotifications } from "../notify/notify.service";
import { autoPrintForOrder } from "../printing/printing.service";
import { earnForOrder, previewRedeem, redeemForOrder, reverseRedeemForOrder } from "../wallet/wallet.service";

/** Shape an order for the wire the same way the REST routes do. */
function wire(o: OrderDoc) {
  return {
    id: String(o._id),
 ...
 */
function firePrinters(ctx: TenantContext, order: OrderDoc, status: OrderStatus): void {
 ...
 */
function fireNotifications(ctx: TenantContext, order: OrderDoc, status: OrderStatus): void {
...
export async function outletConfigFor(ctx: TenantContext) {
...
export const QUEUE_STATUSES: OrderStatus[] = ["placed", "accepted", "preparing"];
 ...
 */
export async function getCapacityStatus(ctx: TenantContext): Promise<CapacityStatus> {
...
export interface QuoteOptions {
  ...
  /** delivery charge in paise, already worked out by the caller */
  deliveryFee?: number;
  /** if given for a delivery channel, the fee is computed from the outlet's
   *  own settings — non-throwing here, so an out-of-range address just quotes
   *  a zero fee and the storefront surfaces the "not serviceable" message */
 ...
 * discount.
 */
export async function quote(ctx: TenantContext, cart: Cart, opts: QuoteOptions = {}): Promise<PricedOrder> {
  const [menu, config] = await Promise.all([getMenu(ctx), outletConfigFor(ctx)]);

  let bareSubtotal: number | null = null;
  const subtotalOf = (): number => {
  ...
  const extras: PriceExtras = { deliveryFee: opts.deliveryFee };
...
export interface DeliveryContext {
  address: { text: string; landmark?: string } & GeoPoint;
}
...
function orderNumberKey(outletId: string): string {
  const d = new Date();
...
export async function placeOrder(input: {
  ...
  tableId?: string;
  sessionId?: string;
  /** required when channel is "delivery" */
  deliveryAddress?: { text: string; landmark?: string; lat: number; lng: number };
  /** who is putting this order in — defaults to "customer", the self-service case */
  placedBy?: PlacedBy;
  /** required when placedBy is "staff" */
  staffId?: string;
  staffName?: string;
}): Promise<OrderDoc> {
  const { ctx } = input;
  const placedBy: PlacedBy = input.placedBy ?? "customer";
  const config = await outletConfigFor(ctx);

  const enabled: Record<OrderChannel, boolean> = {
  ...
  let deliveryRecord: OrderDoc["deliveryAddress"];
...
export async function listOrders(
  ctx: TenantContext,
  opts: { status?: OrderStatus; limit?: number; paymentPending?: "hide" | "only" | "all" } = {},
) {
  const base: Record<string, unknown> = opts.status ? { status: opts.status } : {};
 ...
 */
async function settleWallet(ctx: TenantContext, order: HydratedDocument<OrderDoc>, status: OrderStatus): Promise<void> {
  if (status === "completed") {
...
export async function editOrder(
  ctx: TenantContext,
  id: string,
  input: { cart?: Cart; note?: string; customerName?: string; customerPhone?: string },
  byUserId?: string,
): Promise<OrderDoc> {
  const order = await OrderModel.findOne(tenantFilter(ctx, { _id: id }));
  if (!order) throw new HttpError(404, "Order not found");
  ...
  const LOCK_ORDER: OrderStatus[] = ["placed", "accepted", "preparing", "ready", "completed"];
  if (ops.editLockAfter !== "never") {
    const lockAt = LOCK_ORDER.indexOf(ops.editLockAfter as OrderStatus);
