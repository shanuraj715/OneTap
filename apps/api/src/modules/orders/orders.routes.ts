...
): Promise<{ tableId?: string; sessionId?: string }> {
...
ordersRouter.patch("/:id/status", async (req, res) => {
  const ctx = await requireOutletContext(req, "order:update");
  const user = requireUser(req);
  const status = orderStatusSchema.parse((req.body as { status?: unknown }).status);
...
function shape(o: Record<string, any>, includeCustomer: boolean) {
  return {
    id: String(o._id),
