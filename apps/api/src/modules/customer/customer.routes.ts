...
import { z } from "zod";
import { OutletModel, type CustomerDoc } from "@onetap/db";
import { isProd } from "../../env";
import { HttpError } from "../../middleware/error";
import { customerFromToken, endCustomerSession, requestOtp, verifyOtp } from "./customer.service";
...
async function outletOf(outletId: string) {
...
export async function currentCustomer(req: Request): Promise<CustomerDoc | null> {
...
customerRouter.post("/logout", async (req, res) => {
  await endCustomerSession((req.cookies as Record<string, string> | undefined)?.[CUSTOMER_COOKIE]);
