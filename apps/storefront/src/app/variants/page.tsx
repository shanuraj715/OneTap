...
import type { Layout, MenuItem } from "@onetap/config-schema";
import { VARIANT_SLOTS, type PreviewContext, type VariantMeta } from "@onetap/ui";
  ...
  return (
    <>
      ...
      <main style={page}>
        ...
        {VARIANT_SLOTS.map((slot) => {
          const activeId = (layout as unknown as Record<string, string>)[slot.key];
          return (
            <section key={slot.key} id={slot.key} style={{ marginTop: 46, scrollMarginTop: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                {slot.variants.map((meta: VariantMeta) => (
...
function pickSamples(items: MenuItem[]): MenuItem[] {
...
const page: CSSProperties = { maxWidth: 1140, margin: "0 auto", padding: "56px 24px 96px" };
...
const h1: CSSProperties = { fontSize: "clamp(1.8rem, 4vw, 2.4rem)", margin: "10px 0 8px", color: "var(--color-text)" };
const lede: CSSProperties = { color: "var(--color-text-muted)", maxWidth: "70ch", lineHeight: 1.6, margin: 0 };
const indexNav: CSSProperties = {
  display: "flex",
  gap: 6,
  flexWrap: "wrap",
  marginTop: 22,
...
const h2: CSSProperties = { fontSize: "1.25rem", margin: 0, color: "var(--color-text)" };
const sectionMeta: CSSProperties = { fontSize: "0.8rem", color: "var(--color-text-muted)" };
const frame: CSSProperties = { borderWidth: 1, borderStyle: "solid", borderColor: "var(--color-border)", borderRadius: 12, overflow: "hidden" };
const frameBar: CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: 10,
  flexWrap: "wrap",
...
const frameDesc: CSSProperties = { color: "var(--color-text-muted)", fontSize: "0.78rem" };
