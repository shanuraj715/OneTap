...
const Pin = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
    <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z" />
    <circle cx="12" cy="10" r="3" />
  </svg>
);
const Navigation = ({ size = 15 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
const Alert = ({ size = 15 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
type LocState = "idle" | "locating" | "captured" | "denied" | "unsupported";
...
export function AddressPicker({
  ...
  outletId: string;
  /** cart item total in paise, so the free-delivery threshold shows correctly */
  subtotal: number;
  onChange: (address: DeliveryAddress | null, quote: DeliveryQuote | null) => void;
}) {
  const [loc, setLoc] = useState<LocState>("idle");
  const [point, setPoint] = useState<{ lat: number; lng: number; accuracy?: number } | null>(null);
  ...
  const check = useCallback(
    async (p: { lat: number; lng: number }) => {
      ...
      try {
        ...
        const q = (await res.json()) as DeliveryQuote;
        setQuote(q);
        return q;
      } catch {
        setQuote(null);
  ...
  const publish = useCallback(
    (p: { lat: number; lng: number } | null, addr: string, land: string, q: DeliveryQuote | null) => {
...
const hint: CSSProperties = { fontSize: 12.5, color: "var(--color-text-muted)", margin: "8px 0 0", lineHeight: 1.5 };
...
const capturedChip: CSSProperties = {
  display: "flex",
  alignItems: "center",
  gap: 10,
  padding: "10px 13px",
...
const reshareBtn: CSSProperties = {
  font: "inherit",
  fontSize: 12,
  fontWeight: 600,
  padding: "5px 11px",
