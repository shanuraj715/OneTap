...
import { parseOutletConfig, type OutletConfig } from "@onetap/config-schema";
 ...
 */
export async function getOutlet(): Promise<ResolvedOutlet | null> {
  ...
  try {
    ...
    const { outlet } = (await res.json()) as { outlet: { _id: string; name: string; slug: string; config: unknown } };
    return {
      id: outlet._id,
      name: outlet.name,
      slug: outlet.slug,
...
export async function getOutletById(outletId: string): Promise<ResolvedOutlet | null> {
  try {
