...
import type { ClaimedJob, Outlet } from "./api";
...
export interface StationSettings {
  enabled: boolean;
  targets: PrintTarget[];
  /** identifies this browser in the queue's audit trail */
  clientId: string;
}

function loadSettings(): StationSettings {
  const fallback: StationSettings = {
    enabled: false,
...
function saveSettings(s: StationSettings): void {
  try {
...
function eposEnvelope(escposBase64: string): string {
...
async function printViaEpos(outlet: Outlet, job: ClaimedJob): Promise<void> {
...
function explainEposCode(code: string | undefined): string | null {
  switch (code) {
...
export interface RunnerState {
  ...
  setTargets: (t: PrintTarget[]) => void;
  busy: boolean;
  lastRun: Date | null;
  lastError: string | null;
  /** run one poll immediately, for the "check now" button */
  runNow: () => void;
}
 ...
 */
export function usePrintRunner(outlet: Outlet | undefined, canPrint: boolean): RunnerState {
  ...
  const update = useCallback((patch: Partial<StationSettings>) => {
    setSettings((prev) => {
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          const message = (e as Error).message || "Printing failed";
