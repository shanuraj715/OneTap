...
interface ServerEvent {
  type: "hello" | "order.created" | "order.updated" | "order.deleted" | "print.updated" | "ping";
  order?: { id: string };
  orderId?: string;
  job?: unknown;
}
...
export interface OrderStream {
  status: StreamStatus;
  /** the last order id that arrived new, so the list can flash it */
 ...
 */
export function useOrderStream(outlet: Outlet | undefined, enabled: boolean): OrderStream {
