...
import { ROLE_LABELS, type Permission } from "@onetap/config-schema";
import {
  Bell,
  CreditCard,
  ExternalLink,
...
function Shell({ can }: { can: (p: Permission) => boolean }) {
  const { user } = useAuth();
...
const splash: CSSProperties = { minHeight: "100vh", display: "grid", placeItems: "center", color: "var(--color-text-muted)" };
...
const brand: CSSProperties = { fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: 18 };
const outletChip: CSSProperties = {
  fontSize: 12,
...
const nav: CSSProperties = { display: "flex", flexDirection: "column", gap: 14, marginTop: 8 };
...
const storefrontLink: CSSProperties = {
  display: "inline-flex",
  alignItems: "center",
  gap: 6,
  fontSize: 13,
...
const content: CSSProperties = { padding: "0 32px 28px", width: "100%", minWidth: 0, height: "100%", overflowX: "hidden", overflowY: "auto" };
