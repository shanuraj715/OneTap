...
export function Coupons() {
  ...
  const remove = useMutation({
    mutationFn: (id: string) => api.deleteCoupon(outlet!, id),
...
function CouponEditor({ outlet, coupon, onClose }: { outlet: Outlet; coupon: Coupon | null; onClose: () => void }) {
  const qc = useQueryClient();
  ...
  const rupees = (paise: number | undefined) => (paise ? paise / 100 : 0);
      ...
      >
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <Field label="Type" style={narrow} info="A flat amount off, or a percentage of the item total.">
            <Select value={form.discountType} onChange={(e) => set({ discountType: e.target.value as DiscountType })}>
              {(["fixed", "percent"] as DiscountType[]).map((d) => (
...
const fieldset: CSSProperties = { border: "1px solid var(--color-border)", borderRadius: 10, padding: "12px 14px 14px", margin: "14px 0" };
const legend: CSSProperties = { fontSize: 12.5, fontWeight: 600, padding: "0 6px", display: "inline-flex", alignItems: "center", gap: 6 };
const narrow: CSSProperties = { maxWidth: 180, flex: "1 1 150px" };
