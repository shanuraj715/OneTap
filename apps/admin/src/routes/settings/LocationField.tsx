...
import type { ShopLocation } from "@onetap/config-schema";
import { Crosshair, MapPin } from "lucide-react";
import { Button, Field, InfoHint, TextInput } from "../../ui";

const MAPS_KEY =
  (import.meta.env.VITE_GOOGLE_MAPS_API_KEY as string | undefined) ??
  (import.meta.env.VITE_NEXT_PUBLIC_GOOGLE_MAPS_API_KEY as string | undefined) ??
  "";

/* eslint-disable @typescript-eslint/no-explicit-any */
const gmaps = (): any => (globalThis as any).google?.maps;
...
function loadGoogleMaps(): Promise<void> {
  ...
  mapsPromise = new Promise<void>((resolve, reject) => {
    ...
    (globalThis as Record<string, unknown>)[cb] = () => {
      delete (globalThis as Record<string, unknown>)[cb];
      resolve();
    };
 ...
 */
export function LocationField({ value, onChange }: { value: ShopLocation; onChange: (v: ShopLocation) => void }) {
  const mapRef = useRef<HTMLDivElement>(null);
  const markerRef = useRef<any>(null);
  const geocoderRef = useRef<any>(null);
  const searchRef = useRef<HTMLInputElement>(null);
  const [mode, setMode] = useState<"loading" | "map" | "manual">("loading");
  const [locating, setLocating] = useState(false);
  const [mapsLink, setMapsLink] = useState("");

  const point = value.point;

  const reverse = useCallback((p: { lat: number; lng: number }): Promise<string> => {
    const g = geocoderRef.current;
    const fallback = `${p.lat.toFixed(5)}, ${p.lng.toFixed(5)}`;
    if (!g) return Promise.resolve(fallback);
    return new Promise((resolve) => {
      g.geocode({ location: p }, (r: { formatted_address: string }[] | null, s: string) => {
  ...
  const apply = useCallback(
    async (p: { lat: number; lng: number }, text?: string) => {
