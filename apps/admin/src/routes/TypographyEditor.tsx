...
const CATEGORIES = ["sans", "serif", "display", "mono"] as const;
...
function useFontPreloader() {
  useEffect(() => {
    ...
    const specs = FONT_FAMILIES.map((f) => f.google).filter(Boolean) as string[];
...
export function TypographyEditor() {
  ...
  useEffect(() => {
    if (outlet && !typo) setTypo(outlet.config.typography);
  }, [outlet, typo]);
  ...
  const set = <K extends keyof Typography>(key: K, value: Typography[K]) => setTypo({ ...typo, [key]: value });
          ...
          <Card title="Scale & weight">
            ...
            <Field label="Type scale" hint="How fast headings grow relative to body text" info="The gap between each heading level. A small scale keeps everything close in size and feels calm and dense; a large scale makes headings dominate and feels bold and airy.">
              <div style={{ display: "flex", gap: 8 }}>
                {(Object.keys(TYPE_SCALES) as (keyof typeof TYPE_SCALES)[]).map((k) => (
                  <button
                    key={k}
                    type="button"
                    onClick={() => set("scale", k)}
            ...
            <Field label="Heading letter spacing" info="Extra space between letters in headings. Slightly negative tightens large display type and usually looks better; positive space suits small uppercase labels.">
              <div style={{ display: "flex", gap: 8 }}>
                {(["tight", "normal", "wide"] as const).map((k) => (
...
const pillOn: CSSProperties = { background: "var(--color-primary)", color: "var(--color-on-primary)", borderColor: "var(--color-primary)" };
