...
import type { AdminOrder, AdminPrinter } from "../lib/api";
...
function loadPageSize(): number {
  try {
    ...
    return PAGE_SIZES.includes(raw as (typeof PAGE_SIZES)[number]) ? raw : 10;
  } catch {
    return 10;
  }
}

const OPEN_STATUSES: OrderStatus[] = ["placed", "accepted", "preparing", "ready"];

type PrintSummary = { printed: number; failed: number; pending: number; total: number };
type PaymentFilter = "paid" | "pending" | "failed" | "refunded";
type PrintFilter = "printed" | "failed" | "pending" | "none";
...
const STATUS_TONE: Record<OrderStatus, "ok" | "warn" | "error" | "info" | "neutral"> = {
  placed: "warn",
  accepted: "info",
  preparing: "info",
  ready: "warn",
 ...
 */
const ACTION_COLOR: Record<OrderStatus, string> = {
...
function actionStyle(status: OrderStatus): CSSProperties {
...
export function Orders() {
  ...
  const [prints, setPrints] = useState<PrintFilter[]>([]);
  ...
  const [confirming, setConfirming] = useState<{ order: AdminOrder; status: OrderStatus; manual: boolean } | null>(null);
  ...
  const printSummaryFor = (id: string): PrintSummary | undefined => printStatus.data?.status[id];
  ...
  const rows = all.filter((o) => {
    ...
    if (payments.length && !payments.includes(o.payment.status as PaymentFilter)) return false;

    if (prints.length) {
      const s = printSummaryFor(o.id);
      const state: PrintFilter = !s || s.total === 0 ? "none" : s.failed ? "failed" : s.pending ? "pending" : "printed";
  ...
  return (
    <>
      ...
      {setStatus.error ? <Toast kind="error">{(setStatus.error as Error).message}</Toast> : null}
      {manual.error ? <Toast kind="error">{(manual.error as Error).message}</Toast> : null}
      {printOn.error ? <Toast kind="error">{(printOn.error as Error).message}</Toast> : null}
...
function OrderRow({
  ...
  expanded: boolean;
  onToggleExpand: () => void;
  /** Opens the confirm dialog — nothing here mutates directly. `manual` means
   *  "set this status directly", the mis-tap correction path. */
  onRequestStatus: (status: OrderStatus, manual: boolean) => void;
 ...
 */
function PrintCell({ summary }: { summary?: PrintSummary }) {
...
function LiveBadge({
  ...
  const map = {
    live: { tone: "ok" as const, label: "Live", title: "Connected — orders arrive the moment they're placed." },
    connecting: { tone: "warn" as const, label: "Connecting…", title: "Opening the live connection." },
    offline: {
      tone: "warn" as const,
...
function SlaBanner({
  ...
  sla: ReturnType<typeof useSlaAlerts>;
  onShowLate: () => void;
  soundReady: boolean;
}) {
...
// Sticky, same trick as PageHeader in ui.tsx — the content area has no top
...
const primaryBtn: CSSProperties = { fontSize: 12.5, padding: "7px 13px", whiteSpace: "nowrap" };
...
const detailBox: CSSProperties = { padding: "14px 18px 16px 52px" };
...
const detailTd: CSSProperties = { padding: "6px 8px", borderBottom: "1px solid var(--color-border)", verticalAlign: "top" };
const detailTotals: CSSProperties = {
  display: "flex",
  gap: 18,
  flexWrap: "wrap",
  alignItems: "baseline",
...
const detailNote: CSSProperties = {
  marginTop: 10,
  padding: "8px 11px",
  borderRadius: 8,
  background: "var(--tone-warning-wash)",
...
const confirmBody: CSSProperties = { padding: "22px 22px 16px", textAlign: "center" };
...
const shortcutsHeader: CSSProperties = {
  display: "flex",
  justifyContent: "space-between",
  alignItems: "center",
  gap: 12,
...
const shortcutsHeaderIcon: CSSProperties = {
  display: "grid",
  placeItems: "center",
  width: 32,
  height: 32,
