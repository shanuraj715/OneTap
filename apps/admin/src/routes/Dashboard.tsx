...
const CHANNEL_LABEL: Record<string, string> = { takeaway: "Takeaway", "dine-in": "Dine-in", delivery: "Delivery" };
    ...
    return (
      <>
        ...
        <Card title="No outlet yet">
          ...
          {seed.error ? <Toast kind="error">{(seed.error as Error).message}</Toast> : null}
        </Card>
      </>
    );
  }
  ...
  return (
    <>
      ...
      {stats.isLoading ? (
        ...
        <Card>
          <Toast kind="error">{(stats.error as Error).message}</Toast>
        </Card>
      ) : !widgets.length ? (
        <Card>
...
const WIDGET_ICON: Record<DashboardWidgetId, LucideIcon> = {
...
function WidgetFrame({ id, children }: { id: DashboardWidgetId; children: ReactNode }) {
...
function renderWidget(id: DashboardWidgetId, s: DashboardStats): ReactNode {
  switch (id) {
    case "revenue-today":
    ...
    case "orders-by-status":
      return (
        <DonutChart
          segments={s.ordersByStatus.map((c) => ({ label: ORDER_STATUS_LABELS[c.status as keyof typeof ORDER_STATUS_LABELS] ?? c.status, value: c.count }))}
        />
      );
    case "orders-by-hour":
      return (
...
const grid: CSSProperties = { display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 16 };
