...
import type { OutletConfig, OutletIdentity } from "@onetap/config-schema";
import {
  AlarmClock,
  Bell,
  Bike,
...
type Tab = "business" | "hours" | "alerts" | "orders" | "kitchen" | "receipts" | "features" | "wallet" | "capacity";

const IDENTITY_FIELDS: { key: keyof OutletIdentity; label: string; hint?: string; info: string }[] = [
  {
    key: "name",
...
export function Settings() {
  ...
  const [draft, setDraft] = useState<OutletConfig | null>(null);
  ...
  const setIdentity = (key: keyof OutletIdentity, value: string) => {
    setDraft({ ...draft, identity: { ...draft.identity, [key]: value } });
    setDirty(true);
  };
  const setOps = <K extends keyof typeof ops>(group: K, patchValue: Partial<(typeof ops)[K]>) => {
    setDraft({ ...draft, operations: { ...ops, [group]: { ...ops[group], ...patchValue } } });
    setDirty(true);
  };
  const setFeature = <K extends keyof OutletConfig["features"]>(
    group: K,
    patchValue: Partial<OutletConfig["features"][K]>,
  ) => {
    setDraft({ ...draft, features: { ...draft.features, [group]: { ...draft.features[group], ...patchValue } } });
    setDirty(true);
  };
  const setWallet = (patchValue: Partial<OutletConfig["wallet"]>) => {
    setDraft({ ...draft, wallet: { ...draft.wallet, ...patchValue } });
    setDirty(true);
  };
  const setCapacity = (patchValue: Partial<OutletConfig["capacity"]>) => {
          ...
          <div style={{ marginTop: 18 }}>
            <Row>
              <Field
                ...
                style={narrow}
              >
                <Select value={ops.orders.editLockAfter} onChange={(e) => setOps("orders", { editLockAfter: e.target.value as typeof ops.orders.editLockAfter })}>
...
const narrow: CSSProperties = { maxWidth: 210, flex: "1 1 170px" };
const stickySave: CSSProperties = {
  position: "sticky",
  bottom: 16,
  display: "flex",
  alignItems: "center",
