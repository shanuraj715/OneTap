...
export function Payments() {
  ...
  const [enabled, setEnabled] = useState<Gateway[] | null>(null);
  ...
  const toggle = (g: Gateway) =>
    setEnabled((prev) => (prev!.includes(g) ? prev!.filter((x) => x !== g) : [...prev!, g]));

  const saveEnabled = () =>
    patch.mutate({
      outlet,
      patch: { payments: { ...outlet.config.payments, enabled } as PaymentSettings },
    });
...
const hint: CSSProperties = { fontSize: 12.5, color: "var(--color-text-muted)", margin: "-4px 0 12px", lineHeight: 1.5 };
...
const pillOk: CSSProperties = { background: "var(--tone-success-wash)", color: "var(--tone-success)" };
const pillWarn: CSSProperties = { background: "var(--tone-warning-wash)", color: "var(--tone-warning)" };
