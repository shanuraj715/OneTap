...
import { getAgentToken, type AdminPrinter, type Outlet, type PrinterInput } from "../../lib/api";
...
const TARGET_ICON: Record<PrintTarget, typeof Monitor> = {
  browser: Monitor,
  "epos-lan": Globe,
  cloud: Cloud,
  agent: Cpu,
...
export function PrinterList({
  ...
  const [editing, setEditing] = useState<AdminPrinter | "new" | null>(null);
  ...
  return (
    <>
      ...
      {test.error ? <Toast kind="error">{(test.error as Error).message}</Toast> : null}
...
function PrinterRow({
  ...
  const health = printer.configError
    ? { tone: "warn" as const, text: printer.configError }
    : printer.lastError
      ? { tone: "error" as const, text: printer.lastError }
      : printer.lastOkAt
        ? { tone: "ok" as const, text: `Last printed ${new Date(printer.lastOkAt).toLocaleString()}` }
        : { tone: "neutral" as const, text: "Not used yet" };
          ...
          {printer.autoPrintOn.length ? (
            printer.autoPrintOn.map((s) => (
              <Pill key={s} tone="info">
                auto @ {ORDER_STATUS_LABELS[s as OrderStatus] ?? s}
              </Pill>
            ))
          ) : (
...
const EMPTY: Partial<PrinterInput> = {
  name: "",
  station: "counter",
  docType: "receipt",
  target: "browser",
...
function PrinterEditor({
  ...
  const [form, setForm] = useState<Partial<PrinterInput>>(() =>
    printer
      ? {
          name: printer.name,
  ...
  const setConn = (patch: Partial<PrinterInput["connection"]>) =>
    setForm((f) => ({ ...f, connection: { ...f.connection, ...patch } }));

  const target = (form.target ?? "browser") as PrintTarget;
  const meta = TARGETS[target];
  const thermal = PAPER[(form.paperWidth ?? "80mm") as PaperWidth].continuous;
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const body: Partial<PrinterInput> = {
      ...form,
    ...
    <div style={overlay} onClick={onClose} role="presentation">
      ...
      >
        ...
        <Field
          ...
          hint={STATION_HINTS[(form.station ?? "counter") as PrintStation]}
        >
          <Select
            value={form.station}
            onChange={(e) => {
              const station = e.target.value as PrintStation;
        ...
        <Field
          ...
          hint={DOC_HINTS[(form.docType ?? "receipt") as PrintDocType]}
        >
          <Select value={form.docType} onChange={(e) => set({ docType: e.target.value as PrintDocType, templateId: null })}>
            {PRINT_DOCS.map((d) => (
        ...
        <Field
          ...
          hint={PAPER[(form.paperWidth ?? "80mm") as PaperWidth].hint}
        >
        ...
        <Field
          ...
          hint={meta.hint}
        >
          <Select value={target} onChange={(e) => set({ target: e.target.value as PrintTarget })}>
            {PRINT_TARGETS.map((t) => (
...
function AgentFields({
  ...
  outlet: Outlet;
  printer: AdminPrinter | null;
  form: Partial<PrinterInput>;
  setConn: (p: Partial<PrinterInput["connection"]>) => void;
}) {
    ...
    <>
      ...
      {printer ? (
        <div style={{ marginBottom: 14 }}>
          ...
          {fetchToken.error ? <Toast kind="error">{(fetchToken.error as Error).message}</Toast> : null}
        </div>
      ) : (
...
const smallBtn: CSSProperties = { fontSize: 12.5, padding: "6px 12px", display: "inline-flex", alignItems: "center", gap: 6 };
