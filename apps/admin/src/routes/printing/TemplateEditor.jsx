            ...
            <Button
              ...
              onClick={() =>
                create.mutate(
                  ...
                  { onSuccess: (r) => setSelectedId((r                               ).template.id) },
                )
              }
            >
...
function LogoField({ form, set }                                                                   ) {
  ...
  const convert = async (url        , threshold        , widthPct        ) => {
    if (!url) {
      set({ logoRaster: null });
      setPreview(null);
      return;
        ...
        {block.enabled ? (
          <>
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center" }}>
              <Select value={block.align} onChange={(e) => onChange({ align: e.target.value               })} style={miniSelect} aria-label="Alignment">
                {ALIGNS.map((a) => (
                  <option key={a} value={a}>{a}</option>
                ))}
              </Select>
              <Select value={block.size} onChange={(e) => onChange({ size: e.target.value              })} style={miniSelect} aria-label="Text size">
                {SIZES.map((s) => (
...
function AddBlock({ existing, onAdd }                                                                  ) {
  ...
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 12, flexWrap: "wrap" }}>
      <Select value={key} onChange={(e) => setKey(e.target.value                 )} style={{ fontSize: 13, maxWidth: 230 }}>
...
function Preview({ outlet, template }                                             ) {
  ...
  return (
    <div style={{ position: "sticky", top: STICKY_HEADER_CLEARANCE }}>
      <Card title="Preview" icon={<Eye size={15} />} subtitle={`${paper.label} · ${charsFor(template)} characters wide`}>
        ...
        {render.error ? <Toast kind="error">{(render.error         ).message}</Toast> : null}
      </Card>
    </div>
  );
}
...
const miniSelect                = { fontSize: 12, padding: "4px 7px", maxWidth: 120 };
const fieldset                = {
  border: "1px solid var(--color-border)",
  borderRadius: 10,
