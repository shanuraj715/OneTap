...
import { formatINR, TABLE_STATUS_LABELS, type Table, type TableStatus } from "@onetap/config-schema";
import { getTableQr, rotateTableQr, type ActiveSession } from "../lib/api";
    ...
    <>
      ...
      {qrFor ? (
        <QrPanel
          ...
          deleteError={(remove.error as Error | null)?.message}
        />
      ) : null}
    </>
  );
...
function SessionRow({
  ...
  return (
    <div style={sessionRow}>
      ...
      {move.error ? <Toast kind="error">{(move.error as Error).message}</Toast> : null}
    </div>
  );
}
      ...
      <div className="ot-anim-pop" style={panel} onClick={(e) => e.stopPropagation()} role="dialog" aria-label={`Table ${table.number}`}>
        ...
        {qr ? (
          ...
          <p style={{ textAlign: "center", color: "var(--color-text-muted)" }}>
            {load.isError ? `⚠ ${(load.error as Error).message}` : "Generating…"}
          </p>
        )}
...
function tone(status: TableStatus): CSSProperties {
